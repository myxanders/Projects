...
//If the submit button is pressed
	if($_SERVER["REQUEST_METHOD"] == "POST") {
	 	...
	 	while ($i < $games && $r=mysqli_fetch_array($sql)){
			...
			if ($homegoals == $homescore && $awaygoals == $awayscore){
		 		...
		 		mysqli_query($db, $beep);
		 		mysqli_query($db, "UPDATE `$asched` SET gf = $awayscore, ga = $homescore WHERE week = '$weekid' AND game = '$gameid'");
				 $beep = mysqli_query($db, "UPDATE `$wk` SET HomeScore = $homescore, AwayScore = $awayscore WHERE `$wk`.`index` = $gameid");
				 ...
				 $hrm = "UPDATE `$wk` SET HomeScore = $homescore, AwayScore = $awayscore WHERE `$wk`.`index` = $gameid";
				 $upd = mysqli_query($db, $hrm);
		 	}
	...
	$wld = mysqli_query($db, "SELECT * FROM efl_clubs WHERE league = 'League_Two'");
	for($i=1; $i<=24; $i++){
	...
	$standing = mysqli_query($db, "SELECT * FROM efl_clubs WHERE league = 'League_Two' ORDER BY ppg DESC, pts DESC, gd DESC, gf DESC, nickname ASC");
	$j = 1;
	while($j <= mysqli_num_rows($standing) && $st = mysqli_fetch_array($standing)){
		if ($j == 1){
			$tsho = $st['teamShort'];
			$moop = mysqli_query($db, "UPDATE efl_prorel SET eltpro = '$tsho' WHERE `rank` = 1");
		}
		elseif ($j == 2){
			$tsho = $st['teamShort'];
			$moop = mysqli_query($db, "UPDATE efl_prorel SET eltpro = '$tsho' WHERE `rank` = 2");
		}
		elseif ($j == 3){
			$tsho = $st['teamShort'];
			$moop = mysqli_query($db, "UPDATE efl_prorel SET eltpro = '$tsho' WHERE `rank` = 3");
		}
		elseif ($j == 4){
			$tsho = $st['teamShort'];
			mysqli_query($db, "UPDATE efl_playoff_semis SET home = '$tsho' WHERE match_id = 'elt47'");
		}
		elseif ($j == 5){
			$tsho = $st['teamShort'];
			mysqli_query($db, "UPDATE efl_playoff_semis SET home = '$tsho' WHERE match_id = 'elt56'");
		}
		elseif ($j == 6){
			$tsho = $st['teamShort'];
			mysqli_query($db, "UPDATE efl_playoff_semis SET away = '$tsho' WHERE match_id = 'elt56'");
		}
		elseif ($j == 7){
			$tsho = $st['teamShort'];
			mysqli_query($db, "UPDATE efl_playoff_semis SET away = '$tsho' WHERE match_id = 'elt47'");
		}		
		elseif ($j == 23){
			$tsho = $st['teamShort'];
			$moop = mysqli_query($db, "UPDATE efl_prorel SET eltrel = '$tsho' WHERE `rank` = 1");
		}
		elseif ($j == 24){
			$tsho = $st['teamShort'];
			$moop = mysqli_query($db, "UPDATE efl_prorel SET eltrel = '$tsho' WHERE `rank` = 2");
		}		
		$j++;
	}
?>
...
<html>
	<head>
	  ...
	  <title>League Two Fixtures</title>
...
<body>
	<h1 align="center"><a href='../League_Two' style="text-decoration: none; color:white;">2019-20 League Two Fixtures</a></h1>
		...
		<div id="week<?php echo $k ?>" class="tabcontent">
			<?php
				...
				<form action="schedule.php" method="post" align="center">
					<table align="center" id="eltgames">
						<tr>
						...
						<tr>				
							...
							<td id="<?php echo $row['Home']?>" style="font-weight: bold; text-align: right;"><img src="../../../Assets/EFL/League_Two/<?php echo $row['Home']?>.png" style="float:left;" width=30 height=30><span style="float: right; padding-top: 6px;" width=100;><?php echo $row['Home']?></span></td>
							<td align="center"><?php echo $i+1;?></td>
								<!-- Cell for the home tricode and logo -->
							<td id="<?php echo $row['Away']?>" style="font-weight: bold;"><span style="float: left; padding-top: 6px;"><?php echo $row['Away']?></span><img src="../../../Assets/EFL/League_Two/<?php echo $row['Away']?>.png" style="float: right; text-align: right; vertical-align: bottom;" width=30 height=30></td>
							<td align="center">
