...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($home36_l1 != NULL){
        mysqli_query($db, "UPDATE efl_playoff_semis SET home_leg1 = $home36_l1 WHERE match_id = 'champ36'");
    };
    if ($home36_l2 != NULL){
        mysqli_query($db, "UPDATE efl_playoff_semis SET home_leg2 = $home36_l2 WHERE match_id = 'champ36'");
    };
    if ($home36_pks != NULL){
        mysqli_query($db, "UPDATE efl_playoff_semis SET home_pks = $home36_pks WHERE match_id = 'champ36'");
    };
    if ($away36_l1 != NULL){
        mysqli_query($db, "UPDATE efl_playoff_semis SET away_leg1 = $away36_l1 WHERE match_id = 'champ36'");
    };
    if ($away36_l2 != NULL){
        mysqli_query($db, "UPDATE efl_playoff_semis SET away_leg2 = $away36_l2 WHERE match_id = 'champ36'");
    };
    if ($away36_pks != NULL){
        mysqli_query($db, "UPDATE efl_playoff_semis SET away_pks = $away36_pks WHERE match_id = 'champ36'");
    };
    if ($home45_l1 != NULL){
        mysqli_query($db, "UPDATE efl_playoff_semis SET home_leg1 = $home45_l1 WHERE match_id = 'champ45'");
    };
    if ($home45_l2 != NULL){
        mysqli_query($db, "UPDATE efl_playoff_semis SET home_leg2 = $home45_l2 WHERE match_id = 'champ45'");
    };
    if ($home45_pks != NULL){
        mysqli_query($db, "UPDATE efl_playoff_semis SET home_pks = $home45_pks WHERE match_id = 'champ45'");
    };
    if ($away45_l1 != NULL){
        mysqli_query($db, "UPDATE efl_playoff_semis SET away_leg1 = $away45_l1 WHERE match_id = 'champ45'");
    };
    if ($away45_l2 != NULL){
        mysqli_query($db, "UPDATE efl_playoff_semis SET away_leg2 = $away45_l2 WHERE match_id = 'champ45'");
    };
    if ($away45_pks != NULL){
        mysqli_query($db, "UPDATE efl_playoff_semis SET away_pks = $away45_pks WHERE match_id = 'champ45'");
    };
    if ($homescore != NULL){
        mysqli_query($db, "UPDATE efl_playoff_finals SET homescore = $homescore WHERE match_id = 'champ'");
    };
    if ($home_pks != NULL){
        mysqli_query($db, "UPDATE efl_playoff_finals SET homepks = $home_pks WHERE match_id = 'champ'");
    };
    if ($awayscore != NULL){
        mysqli_query($db, "UPDATE efl_playoff_finals SET awayscore = $awayscore WHERE match_id = 'champ'");
    };
    if ($away_pks != NULL){
        mysqli_query($db, "UPDATE efl_playoff_finals SET awaypks = $away_pks WHERE match_id = 'champ'");
    };

   if ($home36_l2 != NULL) {
        $sql = mysqli_query($db, "SELECT home,away FROM efl_playoff_semis WHERE match_id = 'champ36'");
        ...
        if ($home36_l1 + $home36_l2 + $home36_pks > $away36_l1 + $away36_l2 + $away36_pks) {
            mysqli_query($db, "UPDATE efl_playoff_finals SET home = '$home' WHERE match_id = 'champ'");
        } elseif ($home36_l1 + $home36_l2 + $home36_pks < $away36_l1 + $away36_l2 + $away36_pks) {
            mysqli_query($db, "UPDATE efl_playoff_finals SET home = '$away' WHERE match_id = 'champ'");
        }
    }
    if ($home45_l2 != NULL) {
        $sql = mysqli_query($db, "SELECT home,away FROM efl_playoff_semis WHERE match_id = 'champ45'");
        $r = mysqli_fetch_array($sql);
        $home = $r['home'];
        $away = $r['away'];
        if ($home45_pks == NULL) {
        ...
        if ($home45_l1 + $home45_l2 + $home45_pks > $away45_l1 + $away45_l2 + $away45_pks) {
            mysqli_query($db, "UPDATE efl_playoff_finals SET away = '$home' WHERE match_id = 'champ'");
        } elseif ($home45_l1 + $home45_l2 + $home45_pks < $away45_l1 + $away45_l2 + $away45_pks) {
            mysqli_query($db, "UPDATE efl_playoff_finals SET away = '$away' WHERE match_id = 'champ'");
        }
    }
    if ($homescore != 'NULL') {
        $sql = mysqli_query($db, "SELECT home,away FROM efl_playoff_finals WHERE match_id = 'champ'");
        $r = mysqli_fetch_array($sql);
        $home = $r['home'];
        $away = $r['away'];
        if ($home_pks == 'NULL') {
        ...
        if ($homescore + $home_pks > $awayscore + $away_pks) {
            mysqli_query($db, "UPDATE efl_prorel SET champpro = '$home' WHERE `rank` = 3");
        } elseif ($homescore + $home_pks < $awayscore + $away_pks) {
            mysqli_query($db, "UPDATE efl_prorel SET champpro = '$away' WHERE `rank` = 3");
        }
    }
}
...
<head>
    ...
    <title>EFL Championship Playoffs</title>
...
<body>
    <h1 align="center"><a href='../Championship' style="text-decoration: none; color:white;">2019-20 Championship Playoffs</a></h1>
    ...
    <div id="semis" class="tabcontent" style="display:block;">
        <h3>Championship Playoff Semi-Finals</h3>
        <form action="playoffs.php" method="post" align="center">
            <div id="semifinals" class="champtable">
                <p>Semifinal 3/6</p>
                <?php
                $sql = mysqli_query($db, "SELECT * FROM efl_playoff_semis WHERE match_id = 'champ36'");
                $r = mysqli_fetch_array($sql);
                $home = $r['home'];
            ...
            <div id="semifinals" class="champtable">
                <p>Semifinal 4/5</p>
                <?php
                $sql = mysqli_query($db, "SELECT * FROM efl_playoff_semis WHERE match_id = 'champ45'");
                $r = mysqli_fetch_array($sql);
                $home = $r['home'];
                $query = mysqli_query($db, "SELECT nickname FROM efl_clubs WHERE teamShort = '$home'");
                $q = mysqli_fetch_array($query);
    ...
    <div id="finals" class="tabcontent"">
        <h3>Championship Playoff Finals</h3>
        <form action=" playoffs.php" method="post" align="center">
        <div class=" champtable">
            <p>Finals</p>
            <?php
            $sql = mysqli_query($db, "SELECT * FROM efl_playoff_finals WHERE match_id = 'champ'");
            $r = mysqli_fetch_array($sql);
            $home = $r['home'];
            $query = mysqli_query($db, "SELECT nickname FROM efl_clubs WHERE teamShort = '$home'");
            $q = mysqli_fetch_array($query);
